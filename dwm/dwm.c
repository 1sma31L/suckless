...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		if (splitstatus) {
			...
			apply_fribidi(mstext);
			msx = (m->ww - TEXTW(mstext) + lrpad) / 2; /* x position of middle status text */
			drw_text(drw, msx, 0, TEXTW(mstext) - lrpad, bh, 0, fribidi_text, 0);
		}
		tw = TEXTW(rstext) - lrpad / 2; /* 2px extra right padding */
		drw_text(drw, m->ww - tw - stw, 0, tw, bh, lrpad / 2, rstext, 0);
	}
